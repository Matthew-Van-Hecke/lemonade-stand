...
namespace LemonadeStand_3DayStarter
{
    class Recipe
    {
        ...
        public void AdjustRecipe(Inventory inventory)
        {
            ...
            SetSizeOfBatch(inventory);
            SetNumberOfLemons(inventory);
            SetAmountOfSugar(inventory);
            SetNumberOfIceCubes(inventory);
        }
        public void SetSizeOfBatch(Inventory inventory)
        {
        ...
        public void SetNumberOfLemons(Inventory inventory)
        {
        ...
        public void SetAmountOfSugar(Inventory inventory)
        {
        ...
        public void SetNumberOfIceCubes(Inventory inventory)
        {
        ...
        public bool WouldYouLikeToAdjustRecipe()
        {
            ...
            do
            {
                response = GetStringUserInput();
            } while (response.ToLower() != "yes" && response.ToLower() != "no");
            return CheckIfUserResponseValid(response);
        }
        public string GetStringUserInput()
        {
            string userIn = Console.ReadLine();
            if (userIn.ToLower() != "yes" && userIn.ToLower() != "no")
            {
                Console.WriteLine("Invalid response. Please type either \"yes\" or \"no\".");
            }
            return userIn;
        }
        public bool CheckIfUserResponseValid(string response)
        {
